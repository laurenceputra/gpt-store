...
vi.mock("../src/tools/getMemory", () => ({ getMemory: vi.fn(async () => ({ found: false })) }));
...
describe("mcp wire protocol", () => {
  ...
  it("tools/list returns six tool schemas", async () => {
    const res = await worker.fetch(mcpRequest(JSON.stringify({ jsonrpc: "2.0", id: 2, method: "tools/list" })), env);
    const json = await res.json<any>();
  ...
  it("malformed JSON returns parse error", async () => {
    const res = await worker.fetch(mcpRequest("{"), env);
    const json = await res.json<any>();
