...
const { searchKeywordMock, getMemoryByIdMock, getTagsMock, embedTextMock, queryVectorsMock } = vi.hoisted(() => ({
  ...
  getMemoryByIdMock: vi.fn(async (_env: any, _tenantId: string, _id: string) => null),
  getTagsMock: vi.fn(async (_env: any, _tenantId: string, _id: string) => [] as string[]),
...
describe("search", () => {
  it("filters semantic hits by requested tags while allowing keyword matches", async () => {
    ...
    (getMemoryByIdMock as any).mockImplementation(async (_env: any, _tenantId: string, id: string) => {
    ...
    (getTagsMock as any).mockImplementation(async (_env: any, _tenantId: string, id: string) => {
    ...
    const out = await searchMemory({ query: "hello", tags: ["urgent"] }, { env: {} as any, auth });
  ...
  it("search excludes archived by default", async () => {
    const out = await searchMemory({ query: "hello" }, { env: {} as any, auth });
  ...
  it("passes optional filters to D1 fallback", async () => {
    await searchMemory({ query: "hello", namespace: "team-a", project_key: null, memory_type: "open_item", tags: ["urgent"] }, { env: {} as any, auth });
